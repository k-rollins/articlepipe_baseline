class AccessLevelsController < ApplicationController
  ...
  before_action :check_webmaster_status
  ...
  private
    ...
    def check_webmaster_status
      redirect_to(controller: "pages", action: "home") unless current_user.access_level.admin_level && current_user.access_level.admin_level >= 800
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def access_level_params
      params.require(:access_level).permit(:user_id, :user_level, :frozen, :banned, :admin, :admin_level)
    end
end
